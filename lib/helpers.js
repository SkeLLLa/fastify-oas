  ...
  return Object.keys(obj).reduce((newObj, key) => {
    ...
    if (Array.isArray(val)) {
      if (obj.type === 'array' && key === 'items') {
        ...
        newObj.minItems = obj.minItems === undefined
          ? obj.items.length
          : obj.minItems;
        newObj.maxItems = obj.maxItems === undefined
          ? obj.items.length
          : obj.maxItems;
      } else {
        if (!obj.type && key === 'enum' && obj.enum.every((v) => typeof v === typeof obj.enum[0])) {
    ...
    } else if (obj.type === 'object' && key === 'properties' && typeof val === 'object') {
      // Process nested schemas in object properties
      newObj.properties = Object.keys(obj.properties)
        .reduce((processed, prop) => {
