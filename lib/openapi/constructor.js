...
module.exports = ({ options = {}, getSchemas, getSchema, routes = [] }) => {
  try {
    ...
    for (const routeOpts of routes) {
      ...
      if (schema) {
        ...
        if (schema.querystring) {
          if (schema.querystring.$ref) {
            if (
              schema.querystring.$ref.indexOf('#') ===
              schema.querystring.$ref.length - 1
            ) {
              const schemaId = schema.querystring.$ref.substring(
                0,
                schema.querystring.$ref.indexOf('#')
              );
        ...
        if (schema.body) {
          ...
          if (schema.body.$ref) {
            if (schema.body.$ref.indexOf('#') === schema.body.$ref.length - 1) {
              const schemaId = schema.body.$ref.substring(
                0,
                schema.body.$ref.indexOf('#')
              );
              const resolved = getSchema(schemaId);
              helpers.genBody(
                swaggerMethod.requestBody,
        ...
        if (schema.params) {
          if (schema.params.$ref) {
            if (
              schema.params.$ref.indexOf('#') ===
              schema.params.$ref.length - 1
            ) {
              const schemaId = schema.params.$ref.substring(
                0,
                schema.params.$ref.indexOf('#')
              );
              const resolved = getSchema(schemaId);
        ...
        if (schema.headers) {
          if (schema.headers.$ref) {
            if (
              schema.headers.$ref.indexOf('#') ===
              schema.headers.$ref.length - 1
            ) {
              const schemaId = schema.headers.$ref.substring(
                0,
                schema.headers.$ref.indexOf('#')
              );
              const resolved = getSchema(schemaId);
        ...
        if (schema.cookies) {
          if (schema.cookies.$ref) {
            if (
              schema.cookies.$ref.indexOf('#') ===
              schema.cookies.$ref.length - 1
            ) {
              const schemaId = schema.cookies.$ref.substring(
                0,
                schema.cookies.$ref.indexOf('#')
              );
              const resolved = getSchema(schemaId);
