          ...
          if (refs.length > 0) {
            refs.forEach((ref) => {
              ...
              if (refValue.indexOf('#') === refValue.length - 1) {
                ...
                const ret = {};
                ret[code] = resolved;
                helpers.genResponse(swaggerMethod.responses, ret, schema.produces);
              }
            })
          } else {
            helpers.genResponse(swaggerMethod.responses, schema.response || { 200: { description: 'Default Response' } }, schema.produces);
          }
        } else {
          helpers.genResponse(
